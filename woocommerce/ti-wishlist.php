...
<div class="tinv-wishlist woocommerce tinv-wishlist-clear">
	...
	<form action="<?php echo esc_url( $form_url ); ?>" method="post" autocomplete="off">
		...
		?>
		<ul class="tn-wishlist tinvwl-table-manage-list">
			<?php /*
			<li class="tn-wishlist-head">
				<?php if ( isset( $wishlist_table['colm_checkbox'] ) && $wishlist_table['colm_checkbox'] ) { ?>
					<span class="product-cb"><input type="checkbox" class="global-cb"
				...
				<?php if ( isset( $wishlist_table_row['colm_price'] ) && $wishlist_table_row['colm_price'] ) { ?>
					<span class="product-price"><?php esc_html_e( 'Unit Price', 'ti-woocommerce-wishlist' ); ?></span>
				<?php } ?>
				<?php if ( isset( $wishlist_table_row['colm_date'] ) && $wishlist_table_row['colm_date'] ) { ?>
					<span class="product-date"><?php esc_html_e( 'Date Added', 'ti-woocommerce-wishlist' ); ?></span>
				<?php } ?>
				<?php if ( isset( $wishlist_table_row['colm_stock'] ) && $wishlist_table_row['colm_stock'] ) { ?>
					<span class="product-stock"><?php esc_html_e( 'Stock Status', 'ti-woocommerce-wishlist' ); ?></span>
				<?php } ?>
			...
			<?php */
			do_action( 'tinvwl_wishlist_contents_before' ); ?>
			<?php
			global $product, $post;
			...
			foreach ( $products as $wl_product ) {
				...
				if ( $wl_product['quantity'] > 0 && apply_filters( 'tinvwl_wishlist_item_visible', true, $wl_product, $product ) ) {
					...
					?>
					<li class="tn-whishlist-item <?php echo esc_attr( apply_filters( 'tinvwl_wishlist_item_class', 'wishlist_item', $wl_product, $product ) ); ?>">
						<?php if ( isset( $wishlist_table['colm_checkbox'] ) && $wishlist_table['colm_checkbox'] ) { ?>
							<span class="product-cb">
								<?php
						...
						<?php if ( isset( $wishlist_table_row['colm_stock'] ) && $wishlist_table_row['colm_stock'] ) { ?>
							<span class="product-stock">
								...
								$availability_html = empty( $availability['availability'] ) ?
									...
									'"><span><i class="ftinvwl ftinvwl-check"></i></span><span class="tinvwl-txt">' .
									esc_html__( 'In stock', 'ti-woocommerce-wishlist' ) .
									'</span></p>' :
									'<p class="stock ' . esc_attr( $availability['class'] ) .
									'"><span><i class="ftinvwl ftinvwl-' .
									( ( 'out-of-stock' === esc_attr( $availability['class'] ) ?
										'times' : 'check' ) ) .
									'"></i>' . 
									esc_html( $availability['availability'] ) . '</span></p>';
